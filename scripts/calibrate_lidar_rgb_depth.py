...
# -*- coding: utf-8 -*-
"""
Estimate LiDAR-to-camera extrinsics from RGB, depth, and LiDAR data.

This is the clean baseline calibration script:

* match RGB, depth, and LiDAR frames by timestamp
* search signed LiDAR axis permutations
* refine continuous rotation and translation with Powell optimization
* score LiDAR->RGB with RGB image edges
* score LiDAR->Depth with depth consistency and depth edges
* export best overlays and calibration_summary.json
"""
...
def discover_parts(seq_root: str) -> List[str]:
    ...
    return children if children else [seq_root]


def choose_part_root(seq_root: str, part_index: int) -> str:
    parts = discover_parts(seq_root)
    if not parts:
        raise ValueError(f"No part folders found under {seq_root}")
    if len(parts) == 1:
        return parts[0]
...
def load_depth_image(path: str) -> Tuple[np.ndarray, bool]:
    ...
    if depth.ndim == 3:
        if depth.shape[2] >= 3:
            ...
            if channel_delta > 0:
                ...
                if not getattr(load_depth_image, "_warned_colorized", False):
                    warnings.warn(
                        f"Depth images appear colorized; decoded with {cmap_name} and returned as normalized depth."
                    )
                    load_depth_image._warned_colorized = True  # type: ignore[attr-defined]
                return norm_depth.astype(np.float32), True
            depth = depth[..., 0]
        else:
            depth = depth[..., 0]
    depth = depth.astype(np.float32)
    orig_dtype = np.asarray(depth_raw).dtype
    if np.issubdtype(orig_dtype, np.integer) or float(np.nanmax(depth)) > 50.0:
        scale = _infer_depth_scale_from_data(depth)
        depth = depth / scale
    return depth, False


def estimate_depth_metric_scale(depth: np.ndarray, lidar_points: np.ndarray) -> float:
    ...
    if lidar_points is None or lidar_points.size == 0:
        return 1.0
    lidar_norm = np.linalg.norm(np.asarray(lidar_points, dtype=np.float32), axis=1)
    lidar_norm = lidar_norm[np.isfinite(lidar_norm) & (lidar_norm > 0)]
    if lidar_norm.size == 0:
        return 1.0
    depth_p = float(np.percentile(depth[valid], 90.0))
    lidar_p = float(np.percentile(lidar_norm, 90.0))
    if depth_p <= 1e-8:
        return 1.0
    scale = lidar_p / depth_p
    return float(np.clip(scale, 0.05, 200.0))
...
def compute_edge_map_rgb(rgb: np.ndarray) -> np.ndarray:
    gray = cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY).astype(np.float32) / 255.0
    gx = cv2.Sobel(gray, cv2.CV_32F, 1, 0, ksize=3)
    gy = cv2.Sobel(gray, cv2.CV_32F, 0, 1, ksize=3)
    mag = cv2.magnitude(gx, gy)
    denom = float(np.percentile(mag, 99.0))
    if denom <= 1e-8:
...
def axis_matrix_from_name(mapping_name: str) -> np.ndarray:
    ...
    return np.stack([axis_vectors[name] for name in components], axis=1).astype(np.float32)
...
def project_points(points_cam: np.ndarray, K: np.ndarray, image_size: Tuple[int, int]) -> Dict[str, np.ndarray]:
...
def make_overlay(
    ...
    if len(z) > 0:
        ...
        z_max = float(np.percentile(z, 98.0))
...
def summarize_translation(t: np.ndarray) -> Dict[str, float]:
    return {"x": float(t[0]), "y": float(t[1]), "z": float(t[2]), "norm": float(np.linalg.norm(t))}


def score_rgb_frame(points_cam: np.ndarray, K: np.ndarray, rgb_edge: np.ndarray) -> Tuple[float, Dict[str, float], Dict[str, np.ndarray]]:
    h, w = rgb_edge.shape[:2]
    proj = project_points(points_cam, K, (w, h))
    if proj["projected_count"] == 0:
        return 0.0, {"coverage": 0.0, "edge": 0.0}, proj
    ...
    score = 0.65 * edge_score + 0.35 * coverage
    return score, {"coverage": coverage, "edge": edge_score}, proj
...
def score_depth_frame(
    ...
    proj = project_points(points_cam, K, (w, h))
    if proj["projected_count"] == 0:
        return 0.0, {"coverage": 0.0, "edge": 0.0, "depth": 0.0}, proj
    u = proj["u"].astype(np.int32)
    v = proj["v"].astype(np.int32)
    z_lidar = proj["z"]
    ...
    score = 0.45 * depth_score + 0.30 * edge_score + 0.25 * coverage
    return score, {"coverage": coverage, "edge": edge_score, "depth": depth_score, "depth_err": depth_err}, proj


def score_joint_frame(
    points_cam: np.ndarray,
    rgb_K: np.ndarray,
    rgb_edge: np.ndarray,
    depth_K: np.ndarray,
    ...
    rgb_score, rgb_metrics, rgb_proj = score_rgb_frame(points_cam, rgb_K, rgb_edge)
    depth_score, depth_metrics, depth_proj = score_depth_frame(points_cam, depth_K, depth, depth_edge, 0.25)
    score = 0.55 * rgb_score + 0.45 * depth_score
    coverage = 0.5 * (rgb_metrics.get("coverage", 0.0) + depth_metrics.get("coverage", 0.0))
    edge = 0.5 * (rgb_metrics.get("edge", 0.0) + depth_metrics.get("edge", 0.0))
    metrics = {
        "coverage": coverage,
        "edge": edge,
        "depth": depth_metrics.get("depth", 0.0),
        "depth_err": depth_metrics.get("depth_err", float("inf")),
        "rgb_coverage": rgb_metrics.get("coverage", 0.0),
        "rgb_edge": rgb_metrics.get("edge", 0.0),
        "depth_coverage": depth_metrics.get("coverage", 0.0),
        "depth_edge": depth_metrics.get("edge", 0.0),
        "depth_score": depth_metrics.get("depth", 0.0),
    }
    # Use the projection with more visible points for overlay purposes.
    proj = rgb_proj if rgb_proj["projected_count"] >= depth_proj["projected_count"] else depth_proj
    return score, metrics, proj
...
def apply_candidate_transform(points_lidar: np.ndarray, mapping_name: str, rotvec: np.ndarray, translation: np.ndarray) -> np.ndarray:
    ...
    R_delta = rotvec_to_matrix(rotvec)
    return transform_points(mapped, R_delta, np.asarray(translation, dtype=np.float32))


def build_samples(
    part_root: str,
    rgb_intrinsics_path: str,
    ...
    rgb_files = list_modal_files(rgb_dir, IMAGE_EXTS)
    ...
    if not pcd_files:
        raise ValueError(f"No LiDAR PCD files found in {lidar_dir}")

    rgb_K_raw, rgb_src_size = parse_camera_intrinsics_file(rgb_intrinsics_path)
    depth_K_raw, depth_src_size = parse_camera_intrinsics_file(depth_intrinsics_path)
    rgb_timestamps = np.array([timestamp_ns_from_path(path) for path in rgb_files], dtype=np.int64)
    depth_timestamps = np.array([timestamp_ns_from_path(path) for path in depth_files], dtype=np.int64)
    pcd_timestamps = np.array([timestamp_ns_from_path(path) for path in pcd_files], dtype=np.int64)

    tolerance_ns = int(max(tolerance_sec, 0.0) * 1e9)
    candidate_rows = []
    for rgb_idx, rgb_path in enumerate(rgb_files):
        rgb_ts = int(rgb_timestamps[rgb_idx])
        depth_idx = int(np.argmin(np.abs(depth_timestamps - rgb_ts)))
        pcd_idx = int(np.argmin(np.abs(pcd_timestamps - rgb_ts)))
        depth_diff = int(abs(int(depth_timestamps[depth_idx]) - rgb_ts))
        pcd_diff = int(abs(int(pcd_timestamps[pcd_idx]) - rgb_ts))
        if depth_diff > tolerance_ns or pcd_diff > tolerance_ns:
            continue
        candidate_rows.append(
            (
                rgb_path,
                depth_files[depth_idx],
                pcd_files[pcd_idx],
                rgb_ts,
                int(depth_timestamps[depth_idx]),
                int(pcd_timestamps[pcd_idx]),
            )
        )

    if not candidate_rows:
        raise ValueError("No matched RGB/depth/LiDAR triplets within tolerance.")
    if max_pairs > 0 and len(candidate_rows) > max_pairs:
        indices = np.linspace(0, len(candidate_rows) - 1, num=max_pairs, dtype=int)
        candidate_rows = [candidate_rows[i] for i in indices]

    rng = np.random.default_rng(seed)
    samples: List[SampleTriplet] = []
    for rgb_path, depth_path, pcd_path, rgb_ts, depth_ts, pcd_ts in candidate_rows:
        rgb = load_rgb_image(rgb_path)
        lidar_points = load_pcd_points(pcd_path, max_lidar_points, rng)
        depth, is_colorized = load_depth_image(depth_path)
        if is_colorized:
            depth_scale = estimate_depth_metric_scale(depth, lidar_points)
            depth = (depth * depth_scale).astype(np.float32)
        if depth.shape[:2] != rgb.shape[:2]:
            depth = cv2.resize(depth, (rgb.shape[1], rgb.shape[0]), interpolation=cv2.INTER_NEAREST)

        rgb_edge = compute_edge_map_rgb(rgb)
        depth_edge = compute_edge_map_depth(depth)
        rgb_K = scale_intrinsics(rgb_K_raw, rgb_src_size, (rgb.shape[1], rgb.shape[0]))
...
def estimate_initial_translation(samples: Sequence[SampleTriplet], mapping_name: str, modality: str) -> np.ndarray:
    lidar_centroids = []
    target_centroids = []
...
def estimate_initial_translation_joint(samples: Sequence[SampleTriplet], mapping_name: str) -> np.ndarray:
    lidar_centroids = []
    target_centroids = []
    for sample in samples:
        if sample.lidar_points.size == 0:
        ...
        mapped = apply_axis_mapping(sample.lidar_points, mapping_name)
        lidar_centroids.append(robust_centroid(mapped))
        if sample.depth_points.size > 0:
            target_centroids.append(robust_centroid(sample.depth_points))
...
def aggregate_score(
    samples: Sequence[SampleTriplet],
    mapping_name: str,
    rotvec: np.ndarray,
    ...
    for sample in samples:
        if sample.lidar_points.size == 0:
            continue
        points_cam = apply_candidate_transform(sample.lidar_points, mapping_name, rotvec, t)
        if modality == "rgb":
            score, metrics, proj = score_rgb_frame(points_cam, sample.rgb_K, sample.rgb_edge)
        elif modality == "depth":
            score, metrics, proj = score_depth_frame(points_cam, sample.depth_K, sample.depth, sample.depth_edge, 0.25)
        elif modality == "joint":
            score, metrics, proj = score_joint_frame(
                points_cam,
                sample.rgb_K,
                sample.rgb_edge,
                sample.depth_K,
    ...
    if frame_scores:
        ...
        if modality in ("depth", "joint") and depth_count > 0:
...
def optimize_transform(
    ...
    search_radius: float,
    maxiter: int,
) -> Tuple[np.ndarray, np.ndarray, float, Dict[str, float], List[Dict[str, object]], Dict[str, object]]:
    ...
    return best_rotvec, best_t, float(best_score), best_metrics, frame_details, stats


def calibrate_modality(
    samples: Sequence[SampleTriplet],
    modality: str,
    max_axis_candidates: int,
    search_radius: float,
    maxiter: int,
    use_depth_init_for_rgb: bool,
    search_axis_mapping: bool,
) -> Dict[str, object]:
    quick_rank = []
    init_rotvec = np.zeros(3, dtype=np.float32)
    for mapping_name in signed_permutation_candidates(search_axis_mapping):
    ...
    for cand in keep:
        ...
        if modality == "rgb" and use_depth_init_for_rgb:
            ...
            local_best = None
            for start in starts:
                best_rotvec, best_t, score, metrics, frame_details, opt_stats = optimize_transform(
...
def calibrate_joint_modality(
    ...
    init_rotvec = np.zeros(3, dtype=np.float32)
    for mapping_name in signed_permutation_candidates(search_axis_mapping):
        init_t = estimate_initial_translation_joint(samples, mapping_name)
        score, _, _ = aggregate_score(samples, mapping_name, init_rotvec, init_t, "joint")
        quick_rank.append(
            {
                "mapping": mapping_name,
                "init_rotvec": init_rotvec.tolist(),
                "init_t": init_t.tolist(),
    ...
    for cand in keep:
        ...
        init_rotvec = np.array(cand.get("init_rotvec", [0.0, 0.0, 0.0]), dtype=np.float32)
        best_rotvec, best_t, score, metrics, frame_details, opt_stats = optimize_transform(
            samples, mapping_name, "joint", init_rotvec, init_t, search_radius, maxiter
        )
        record = {
            "mapping": mapping_name,
            "rotvec": best_rotvec.tolist(),
...
def build_overlay_for_best(
    ...
    for sample in samples:
        if sample.lidar_points.size == 0:
            continue
        points_cam = apply_candidate_transform(sample.lidar_points, mapping_name, rotvec, translation)
        if modality == "rgb":
            score, metrics, proj = score_rgb_frame(points_cam, sample.rgb_K, sample.rgb_edge)
            image = sample.rgb
            title = f"RGB best | mapping={mapping_name}"
        elif modality == "depth":
            score, metrics, proj = score_depth_frame(points_cam, sample.depth_K, sample.depth, sample.depth_edge, 0.25)
            valid_depth = sample.depth[np.isfinite(sample.depth) & (sample.depth > 0)]
            depth_scale = float(np.percentile(valid_depth, 95.0)) if valid_depth.size > 0 else 1.0
            depth_scale = max(depth_scale, 1e-6)
            depth_vis = cv2.applyColorMap(
                (np.clip(sample.depth / depth_scale, 0.0, 1.0) * 255.0).astype(np.uint8),
                cv2.COLORMAP_TURBO,
            )
            image = cv2.cvtColor(depth_vis, cv2.COLOR_BGR2RGB)
            ...
            image = sample.rgb
            title = f"Joint best | mapping={mapping_name}"
        if score > best_score:
            best_score = score
            best_sample = sample
            best_proj = proj
    ...
    if best_sample is None or best_proj is None or best_metrics is None or best_image is None:
    ...
    return overlay, {k: float(v) for k, v in best_metrics.items()}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Calibrate LiDAR extrinsics against RGB and depth.")
    parser.add_argument("--seq_root", type=str, default="/add02/users/xuyh/seq1/", help="Sequence root or a single part directory.")
    parser.add_argument("--part_index", type=int, default=0, help="Part index when seq_root contains multiple parts.")
    parser.add_argument("--rgb_dir", type=str, default="", help="Override RGB directory.")
    parser.add_argument("--depth_dir", type=str, default="", help="Override depth directory.")
    parser.add_argument("--lidar_dir", type=str, default="", help="Override LiDAR directory.")
    parser.add_argument("--rgb_intrinsics_path", type=str, default="", help="Override RGB intrinsics file.")
    parser.add_argument("--depth_intrinsics_path", type=str, default="", help="Override depth intrinsics file.")
    parser.add_argument("--output_dir", type=str, default="output/lidar_calibration")
    ...
    parser.add_argument("--maxiter", type=int, default=100, help="Powell iterations per candidate.")
    parser.add_argument(
        "--search_axis_mapping",
        action="store_true",
        default=False,
        help="Search signed LiDAR axis permutations. Disable when LiDAR is already aligned.",
    )
    parser.add_argument("--use_depth_init_for_rgb", action="store_true", default=True, help="Use depth centroid init for RGB.")
    parser.add_argument("--no_depth_init_for_rgb", action="store_false", dest="use_depth_init_for_rgb")
    parser.add_argument(
        "--output_mode",
        type=str,
        default="joint",
        choices=("joint", "rgb", "depth"),
        help="Optimization target. joint uses aligned RGB+depth together.",
    )
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--save_top_k", type=int, default=3, help="Print top-k quick-ranked axis candidates.")
    return parser.parse_args()


def resolve_paths(args: argparse.Namespace) -> Tuple[str, str, str, str, str, str]:
    part_root = choose_part_root(args.seq_root, getattr(args, "part_index", 0))
    rgb_dir = args.rgb_dir or os.path.join(part_root, "rgb")
    depth_dir = args.depth_dir or os.path.join(part_root, "depth")
    lidar_dir = args.lidar_dir or os.path.join(part_root, "lidar")

    rgb_intrinsics = args.rgb_intrinsics_path or next(
        (
            path
            for path in [
                os.path.join(part_root, "color_camera_intrinsics.txt"),
                os.path.join(os.path.dirname(part_root), "color_camera_intrinsics.txt"),
                os.path.join(args.seq_root, "color_camera_intrinsics.txt"),
            ]
            if os.path.exists(path)
        ),
        "",
    )
    depth_intrinsics = args.depth_intrinsics_path or next(
        (
            path
            for path in [
                os.path.join(part_root, "depth_camera_intrinsics.txt"),
                os.path.join(os.path.dirname(part_root), "depth_camera_intrinsics.txt"),
                os.path.join(args.seq_root, "depth_camera_intrinsics.txt"),
            ]
            if os.path.exists(path)
        ),
        "",
    )
    if not rgb_intrinsics:
        raise ValueError("Could not locate RGB intrinsics file. Provide --rgb_intrinsics_path.")
    if not depth_intrinsics:
        raise ValueError("Could not locate depth intrinsics file. Provide --depth_intrinsics_path.")
    return part_root, rgb_dir, depth_dir, lidar_dir, rgb_intrinsics, depth_intrinsics
...
def main() -> None:
    ...
    part_root, rgb_dir, depth_dir, lidar_dir, rgb_intrinsics, depth_intrinsics = resolve_paths(args)
    print(f"[Input] part_root={part_root}")
    print(f"[Input] rgb_dir={rgb_dir}")
    print(f"[Input] depth_dir={depth_dir}")
    print(f"[Input] lidar_dir={lidar_dir}")
    print(f"[Input] rgb_intrinsics={rgb_intrinsics}")
    print(f"[Input] depth_intrinsics={depth_intrinsics}")

    samples = build_samples(
        part_root=part_root,
        rgb_intrinsics_path=rgb_intrinsics,
        depth_intrinsics_path=depth_intrinsics,
    ...
    print(f"[Data] matched triplets: {len(samples)}")
    if not samples:
        raise RuntimeError("No matched samples available.")

    if args.output_mode == "joint":
        result = calibrate_joint_modality(
            samples=samples,
        ...
        result = calibrate_modality(
            samples=samples,
            modality=args.output_mode,
            max_axis_candidates=args.max_axis_candidates,
            search_radius=args.search_radius,
            maxiter=args.maxiter,
            use_depth_init_for_rgb=args.use_depth_init_for_rgb,
            search_axis_mapping=args.search_axis_mapping,
        )

    result_t = np.array(result["translation"], dtype=np.float32)
    result_rotvec = np.array(result["rotvec"], dtype=np.float32)
    overlay, overlay_metrics = build_overlay_for_best(samples, result["mapping"], result_rotvec, result_t, args.output_mode)

    overlay_path = os.path.join(args.output_dir, f"best_{args.output_mode}_overlay.png")
    ...
    summary = {
        ...
        "args": vars(args),
        "target_frame": "aligned_rgb_depth_camera",
        "output_mode": args.output_mode,
        "mapping": result["mapping"],
        "rotvec": result_rotvec.tolist(),
        "translation": summarize_translation(result_t),
        "score": float(result["score"]),
        "metrics": {
            k: float(v) if isinstance(v, (int, float, np.floating, np.integer)) else v
            for k, v in result["metrics"].items()
        },
        "T_lidar_to_cam": compose_transform(result["mapping"], result_rotvec, result_t).tolist(),
        "T_lidar_to_rgb": compose_transform(result["mapping"], result_rotvec, result_t).tolist(),
        "T_lidar_to_depth": compose_transform(result["mapping"], result_rotvec, result_t).tolist(),
        "lidar": {
            "mapping": result["mapping"],
        ...
        "quick_rank": result["quick_rank"],
        "samples": [
            {
                "rgb_path": sample.rgb_path,
                "depth_path": sample.depth_path,
                "pcd_path": sample.pcd_path,
                "rgb_ts_ns": sample.rgb_ts_ns,
                "depth_ts_ns": sample.depth_ts_ns,
                "pcd_ts_ns": sample.pcd_ts_ns,
            }
            for sample in samples
        ],
    }
    ...
    print(
        f"[Result][{args.output_mode.upper()}] mapping={result['mapping']} score={result['score']:.4f} "
        f"rotvec={result_rotvec.tolist()} t={result_t.tolist()}"
    )
    print(f"[Translation] LiDAR -> camera = {result_t.tolist()} (norm={float(np.linalg.norm(result_t)):.4f} m)")
    opt_stats = result.get("opt_stats", {})
    if opt_stats:
        print(
            f"[Opt] success={opt_stats.get('success')} status={opt_stats.get('status')} "
            f"nit={opt_stats.get('nit')} nfev={opt_stats.get('nfev')} fun={opt_stats.get('fun'):.6f}"
        )
        msg = opt_stats.get("message", "")
        if msg:
            print(f"[Opt] message={msg}")

    if args.save_top_k > 0:
        for idx, cand in enumerate(result["quick_rank"][: args.save_top_k]):
            print(f"[Top] {idx}: {cand['mapping']} quick_score={cand['quick_score']:.4f}")
